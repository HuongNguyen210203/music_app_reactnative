...
const MusicPlayer = ({ navigation }) => {
    const route = useRoute();
    const { title, audioUrl, imageUrl, artist, id } = route.params || {}; // Fallback to empty object
    ...
    const [currentImage, setCurrentImage] = useState(imageUrl || "https://path/to/welcomeImage.png"); // Default welcome image
    const [currentArtist, setCurrentArtist] = useState(artist || "Choose a Song to Play");
    const [currentSongId, setCurrentSongId] = useState(id || null);  // Track current song ID
...
const styles = StyleSheet.create({
    ...
    welcomeTitle: {
        fontSize: 28,
        color: '#FFA500', // Bright color to catch attention
        fontWeight: 'bold',
        marginBottom: 10,
    },
    welcomeSubtitle: {
